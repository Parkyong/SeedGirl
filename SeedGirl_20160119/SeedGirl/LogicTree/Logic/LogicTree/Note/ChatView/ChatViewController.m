...
- (void)customNavigationBar{
    //leftbarButtonitem
    UIImage *leftImage = [[UIImage alloc] initWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"feedbackBack.png"]];
    ...
    [button addTarget:self action:@selector(popCurrentPageAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)popCurrentPageAction{
    if ([self.conversation loadAllMessages].count == 0) {
        [[EaseMob sharedInstance].chatManager removeConversationByChatter:self.chatter
...
- (void)setupBarButtonItem
{
    ...
    if (self.isChatGroup) {
        ...
        [clearButton setImage:[UIImage imageNamed:@"chat_button_delete"] forState:UIControlStateNormal];
...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    if ([[EaseMob sharedInstance].chatManager loadTotalUnreadMessagesCountFromDatabase] == 0) {
        [PushNotificationManager manager].hasNewNoteMessage = NO;
    }
}
...
- (void)moreViewFileTransferAction:(DXChatBarMoreView *)moreView{
    [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithBool:YES] forKey:@"isShowPicker"];
    // 隐藏键盘
    [self keyBoardHidden];
...
- (void)removeAllMessages:(id)sender
{
    ...
    if ([sender isKindOfClass:[NSNotification class]]) {
        ...
        [EMAlertView showAlertWithTitle:nil//NSLocalizedString(@"prompt", @"Prompt")
                                message:@"是否清空所有聊天记录"//NSLocalizedString(@"sureToDelete", @"please make sure to delete")
                        ...
                        } cancelButtonTitle:@"取消"//NSLocalizedString(@"cancel", @"Cancel")
                      otherButtonTitles:@"确定"/*NSLocalizedString(@"ok", @"OK")*/, nil];
    }
}
